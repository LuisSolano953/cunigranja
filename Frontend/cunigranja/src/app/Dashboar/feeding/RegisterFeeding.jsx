...
const RegisterFeeding = ({ refreshData, onCloseForm }) => {
  ...
  const extractErrorMessage = (error) => {
    ...
    // Si es un string, devolverlo directamente
    if (typeof error === "string") return error
>>>>>>> 5778637353c2db992520ee2ba3a3b42b97ccbb83
...
<<<<<<< HEAD
            (item) =>
              item.saldo_existente > 0 && item.estado_food !== "Inactivo"
          );
        ...
        if (
          ...
          err.response.config.url.includes("GetAvailableFood")
        ) {
          try {
  ...
  const handleFoodChange = (foodId) => {
    ...
    if (foodId) {
      ...
      const foodSelected = food.find(
        (item) => item.Id_food?.toString() === foodId.toString()
      );

      // Si no se encuentra, intentar con id_food (con i minúscula)
      if (!foodSelected) {
        const foodSelectedAlt = food.find(
          (item) => item.id_food?.toString() === foodId.toString()
        );
...
<<<<<<< HEAD
      ...
      if (cantidadAlimentacion > saldoExistente) {
        setErrorMessage(
          `No hay suficiente alimento. Saldo disponible: ${formatDecimal(
            saldoExistente
          )} g`
        );
        setShowErrorAlert(true);
...
<<<<<<< HEAD
      const response = await axiosInstance.post(
        "/Api/Food/RecalculateFoodBalance",
        {
          id_food: foodId,
          new_saldo: newSaldo,
        }
...
<<<<<<< HEAD
        console.log(
          "Error de transacción detectado, intentando enfoque alternativo..."
        );
...
<<<<<<< HEAD
      ...
      const fechaFormateada = `${fechaObj.getFullYear()}-${String(
        fechaObj.getMonth() + 1
...
<<<<<<< HEAD
                  <option
                    key={item.id_rabbit || item.Id_rabbit}
                    value={item.id_rabbit || item.Id_rabbit}
                  >
=======
...
<<<<<<< HEAD
            {isLoading.rabbit && (
              <p className="text-sm text-gray-500 mt-1">Cargando conejos...</p>
            )}
          </div>
...
<<<<<<< HEAD
              <small className="text-gray-500">
                Disponible: {formatDecimal(selectedFood.saldo_existente)} g
              </small>
=======
...
<<<<<<< HEAD
                  <option
                    key={user.id_user || user.Id_user}
                    value={user.id_user || user.Id_user}
                  >
=======
...
<<<<<<< HEAD
          ...
          <div>
            <label className="block text-gray-700 font-medium mb-2">
              Existencia después de alimentación:
            </label>
=======
...
<<<<<<< HEAD
            <small className="text-gray-500">
              Saldo restante después de la alimentación
            </small>
=======
